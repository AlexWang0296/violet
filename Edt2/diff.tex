...
%DIF DEL 1stEdt.tex   Tue Nov 27 17:04:12 2018
%DIF ADD 2ndEdt.tex   Tue Dec 25 11:02:11 2018
...
%DIF -------
 %DIF > 
% Authors, for metadata in PDF %DIF > 
%\AuthorNames{ } %DIF > 
% frcly@163.com(R.F.);15620864891@163.com(M.W.); y5217@163.com(H.L.);13893591491@163.com(Y.Q.);18350122524@163.com(Q.W);zhiy_rui@163.com(Z.R) %DIF > 
% Affiliations / Addresses (Add [1] after \address if there is only one affiliation.) %DIF > 
\address{% %DIF > 
%$^{1}$ \quad School of Mechanical and Electronical Engineering, Lanzhou University of Technology, Lanzhou 730050, China; frcly@163.com (R.F.); 15620864891@163.com (M.W.)\\ %DIF > 
%DIF -------
...
Deformation phenomena of Ti-Al alloys have been widely studied in order to overcome the problems associated with the \DIFdelbegin \DIFdel{limited }\DIFdelend \DIFaddbegin \DIFadd{poor }\DIFaddend ductility and damage tolerance.  Much of the work has been performed on single phase $\gamma$-TiAl\DIFdelbegin \DIFdel{alloys }\DIFdelend \DIFaddbegin \DIFadd{, $\rm{Ti_3Al}$ alloys, }\DIFaddend and polysynthetically twinned crystals(PTC)\cite{Appel2016}. \DIFaddbegin \DIFadd{Experiments reveals that  the dominant deformation mode in $\rm{Ti_3Al}$ at low temperature is slip activity without any twinning, and fracture type is cleavage\mbox{%DIFAUXCMD
\cite{sastry1980plastic}}%DIFAUXCMD
. Around 1990s, experimental studies on the two phase lamellae Ti-Al alloys show that  mechanical twin and glide of  dislocation are major sources of deformation\mbox{%DIFAUXCMD
\cite{Farenc1993,Appel2005,Appel1998a}}%DIFAUXCMD
. }\DIFaddend Rapture failure at the macroscopic scale can be attributed to abrupt nucleation, growth and propagation of cracks, but at the microscopic scale\DIFaddbegin \DIFadd{, }\DIFaddend defects are initially formed in the casting process, such as voids and inclusions \cite{Tang2014}.Mess research  cover a wide range of factors such as alloy composition, microstructure and deformation temperature, some reports come up with the idea that two phase titanium aluminum alloys with proper phase distribution and grain size exhibit better mechanical performance compared with monolithic constituents $\gamma$(TiAl) and $\alpha_2$($\rm Ti_3Al$) alloy \cite{Kim1995}. \DIFaddbegin \DIFadd{In situ experiments have shown that TiAl and $\rm{Ti_3Al}$ in two phase alloy exhibit different properties comparing in single phase alloys\mbox{%DIFAUXCMD
\cite{Singh2006}}%DIFAUXCMD
. }\DIFaddend void can be arose due to specific volume differences induced by precipitation, different thermal expansion or shrinkage upon heating or cooling the specimen. It has been known that nucleation, growth and coalescence of voids are deemed as the primary mechanism of ductile material fracture, in which void growth is particularly important \cite{Hempel2017a}. The initiation of crack at microscopic scale is a dynamic process, which results in difficulties on study of  mechanism about deformation and cracking, these defects are known playing a fundamental role in the deformation of materials. Multiscale method has been applied to study deformation behavior of polycrystal with single aluminum \cite{Groh2009} and titanium element respectively \cite{Liu2018}. It's necessary to carefully examine the revolution of defects and its influence on the fracture process at atomic scale. 
...
\cite{Kiener2011}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Kiener2011b}}%DIFAUXCMD
...
\DIFdelbegin %DIFDELCMD < \subsection{Model Creation of Crystalline}
%DIFDELCMD < %%%
...
Two phase Ti-Al alloy is composed of $\gamma $-TiAl and $\alpha_2$-$\rm Ti_3Al$\DIFaddbegin \DIFadd{, TiAl }\DIFaddend has a fcc type cell with an $L1_0$ structure, and \DIFdelbegin \DIFdel{$\alpha_2$-}\DIFdelend $\rm Ti_3Al$ has a hcp structure, these two types of initial cells are shown in Fig. \ref{fig:tial-cell}\DIFdelbegin \DIFdel{, constructing }\DIFdelend \DIFaddbegin \DIFadd{. Geometry }\DIFaddend parameters of the two type of unit cell are given by table \ref{tab:lattice_parameter}. Periodic boundary conditions (PBC) \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend applied along three directions, which makes polycrystal with periodic nanovoid structures. The initial \DIFdelbegin \DIFdel{dimension of simulation cell }\DIFdelend \DIFaddbegin \DIFadd{size of simulation box }\DIFaddend is $L_x =200$ \si{\angstrom}, $L_y = $180\si{\angstrom}, $L_z = 210$ \si{\angstrom}, and each model  \DIFdelbegin \DIFdel{contains }\DIFdelend about 460,000 atoms. The model \DIFdelbegin \DIFdel{contains }\DIFdelend \DIFaddbegin \DIFadd{includes }\DIFaddend 6 grains, with random shape and orientation \DIFdelbegin \DIFdel{were created with }\DIFdelend \DIFaddbegin \DIFadd{created by }\DIFaddend Voronoi method. \DIFdelbegin \DIFdel{Uniaxial load was applied to the model at }\DIFdelend \DIFaddbegin \DIFadd{Total simulation time is restricted to computer power, thus strain rate in molecular dynamic simulation is much greater than physical experiments. Attempts have been done to find decent time step and strain rate for tensile test with MD simulation. If the strain rate is too high and time step is too large in the simulation, the model cannot predict the real behavior of dislocations.  Previous work \mbox{%DIFAUXCMD
\cite{Zhu2008,Wu2016}  }%DIFAUXCMD
reveals that }\DIFaddend a strain rate \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{from  $10^8\ \rm{s}^{-1}$ to $10^9\ \rm{s}^{-1}$ is appropriate for system composed of metal atoms like Ti-Al metallic materials. Thus the strain rate of uniaxial loading was set as }\DIFaddend $5\times10^8\ \rm{s}^{-1}$.  
\DIFaddbegin 

\DIFaddend % In order to study the deformation mechanism of the two phase alloy and the effect of void, three types of models were created: Type-1.model without any void; Type-2. models with different size void inside $\alpha_2$ phase; Type-3. models with void at $\alpha_2-\gamma$ interface. \ref{tab:lattice_parameter}. The simulation cells of two phase polycrystalline with an initially spherical void at different position are shown in figure .
\subsection{Analysis method}
In order to identify typical defects in the deformed model, a hybrid analysis method was used with free code ovito\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Stukowski2010a}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Stukowski2010}}%DIFAUXCMD
}\DIFaddend .Dislocation is visualized by DXA method, and centro-symmetry parameter(CSP) is used to differ grain boundaries\DIFdelbegin \DIFdel{from  }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend $\alpha_2$ phase \DIFaddbegin \DIFadd{grains }\DIFaddend and $\gamma$ phase \DIFaddbegin \DIFadd{grains}\DIFaddend . The definition of CSP is as following:
	\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < 	%%%
\DIFdelend \begin{equation} \label{eq:csp} 
	P = \displaystyle\sum_{i=1}^{6}|\vec{R_i}+{\vec{R}}_{i+6}|^2
	\end{equation}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend where $\vec{R_i}$ and ${\vec{R}}_{i+6}$ are the vectors corresponding to the six pairs of opposite nearest neighbors in the fcc lattice. The centro-symmetry parameter(CSP) is zero for atoms in a perfect lattice. In other words, if the lattice is distorted, the value of P will not be zero. Instead, the parameter will have a value within the range corresponding to a particular defect. By removing all the perfect and surface atoms within the bulk, atoms around defected zone are visualized. 
 \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \subsection{Model Verification}
\DIFadd{The results of molecular dynamics simulation greatly rely on accuracy of potential and controlling parameters such as strain rate, temperature and relaxation time. Thus we performed model verification on a simplified model and then compared the results with experiments and simulation done by others.
 }\begin{figure}[ht]
 	\centering
 	\includegraphics[width=1\linewidth]{img/cold-fracture2}
 	\caption{\DIFaddFL{(a) Verifying model: TiAl single crystal bulk (b) Cracked surface of atomic model}\\ \DIFaddFL{(c) Cracked surface from experiment\mbox{%DIFAUXCMD
\cite{CHEN2015365}}%DIFAUXCMD
}}
 	\label{fig:verify}
 \end{figure}
 \DIFadd{uniaxial tensile loading  was applied to single crystal $\gamma$-TiAl bulk shown by Fig. \ref{fig:verify}(a), potential and relaxation parameters are as same as which applied to the polycrystalline model. When temperature is 300K, the sample was tensioned along }[\DIFadd{0 0 1}] \DIFadd{direction at a constant stain rate of $5\times10^8\ \rm{s}^{-1}$. The yield strength of the single crystal TiAl alloy is 9.5 GPa  and that is in good agreement with  work by Tang\mbox{%DIFAUXCMD
\cite{Tang2010} }%DIFAUXCMD
in consideration of size effect. And the snapshoot of the fracture surface is shown by Fig. \ref{fig:verify}, and compared with experiment results by Cao\mbox{%DIFAUXCMD
\cite{CHEN2015365} }%DIFAUXCMD
shown in Fig. \ref{fig:verify}(c). It should be noticed that the image from SEM is in a greater scale than our simulation cell. However, the cracked surface in atomic model exhibits two major characteristics of TiAl alloy. First, the fracture type of TiAl at room temperate is typical brittle failure. Second, the fracture surface  of atomic model in Fig. \ref{fig:verify} (b) is typical cleavage surface. This verification case has proven that the EAM potential is efficiency and accurate enough, and the simulation work flow is valid to predict failure behavior of  the system composed of Ti-Al.
}\DIFaddend \section{Results and Discussion}\label{section:RD}
...
\DIFaddbegin \subsection{Deformation Mechanism of Two Phase Ti-Al Alloy }


\DIFaddend % Due to this effect ( $\alpha_2$ + $\gamma$ ) alloys exhibit some remarkable properties that are unlike those of either constituent.
\DIFdelbegin %DIFDELCMD < \subsection{Deformation Mechanism of Two Phase TiAl Alloy without voids}
%DIFDELCMD < %%%
\DIFdel{Snapshots of atom }\DIFdelend %DIF >  \subsection{Plastic }
\DIFaddbegin \DIFadd{During elastic stage, the deformation of $\alpha_2$ phase and $\gamma$ phase grains is compatible, however the properties of two phases are different. The elastic constants given by  experiments\mbox{%DIFAUXCMD
\cite{Schwarz1995,Tanaka1996} }%DIFAUXCMD
are listed in  table \ref{tab:elastic}. $C_{11}^{\alpha_2}$ is 176, which is smaller than $C_{11}^{\gamma}$, other 5 parameters but $C_{33}$ of $\rm{Ti_3Al}$ are also smaller than TiAl. In order to quantify the difference of the two phase, deformation gradient was calculated by formulation:
}\begin{equation}
...
\end{equation}
\DIFadd{we computes the atomic level elastic strain and deformation gradient tensors in crystalline systems with the numerical method proposed by \mbox{%DIFAUXCMD
\cite{Singh2006}}%DIFAUXCMD
. When global strain is 0.015, local elastic strain $F_{xx}$ of the model is measured along the line cross center of the model and shown by Fig. \ref{fig:strain}. In te figure, $\alpha_2$ phase located in the central part of the axial, and other parts on the axial are $\gamma$ phase.  It should be noticed that atomic level elastic strain effected by random thermal displacements of atoms, thus fluctuation value exists in Fig. \ref{fig:strain}. The local strain of $\alpha_2$ phase grain is greater than $\gamma$ phase grain when global strain of model is 0.015. From the value of local strain we can conclude that $\gamma$ phase less deformable than $\alpha_2$ phase during elastic stage.
	...
	\caption{\DIFaddFL{Local strain along tensile direction }}
	\label{fig:strain}
\end{figure}

\begin{table}[ht]
	\caption{\DIFaddFL{Elastic constants of TiAl and $\rm{Ti_3Al}$}}
	\centering
	\begin{tabular}{l c c c c c c c c c c}
		\toprule
		\textbf{ } &\DIFaddFL{$C_{11}$ }&\DIFaddFL{$C_{12}$ }&\DIFaddFL{$C_{13}$ }&\DIFaddFL{$C_{33}$ }&\DIFaddFL{$C_{44}$ }&\DIFaddFL{$C_{66}$}\\		 
		\midrule
		\DIFaddFL{TiAl\mbox{%DIFAUXCMD
\cite{Schwarz1995} }%DIFAUXCMD
}&\DIFaddFL{186 }&\DIFaddFL{72 }&\DIFaddFL{74 }& \DIFaddFL{176 }&\DIFaddFL{101 }&\DIFaddFL{77	 }\\
		\midrule
		%DIF >  Time/ps	& 0 & 0.15 & 0.16 & 0.17 & 0.18 & 0.19 & 0.Fu = sin20 & 0.21 & 0.22 & 0.23 \\
		%DIF >  \midule
		\DIFaddFL{$\rm{Ti_3Al}$\mbox{%DIFAUXCMD
\cite{Tanaka1996}	}%DIFAUXCMD
}& \DIFaddFL{176 }&  \DIFaddFL{87.8 }& \DIFaddFL{61.2 }& \DIFaddFL{218.7 }& \DIFaddFL{91.9 }& \DIFaddFL{62.4   }\\
		\bottomrule
	\end{tabular} 
	\label{tab:elastic}
\end{table}
%DIF > \begin{equation}
%DIF > 	\label{eq:strin-grade2}
%DIF > 		\mathbf{e} = 1 / 2 \left[ I - \left( \mathbf{F ^ { e }} \right) ^ { - T } \left(\mathbf{ F ^ { e }} \right) ^ { - 1 } \right]
%DIF > \end{equation}
%DIF > \begin{equation}
%DIF > \begin{aligned} d $\mathbf { x } & = \frac { \partial \mathbf { x } } { \partial \mathbf { X } } d \mathbf { X } \\ & = \nabla \chi ( \mathbf { X } , t ) d \mathbf { X } = \mathbf { F } ( \mathbf { X } , t ) d \mathbf { X }$
%DIF > \end{aligned} 
%DIF > \end{equation}
\DIFadd{Snapshots of atoms }\DIFaddend configuration at the 3 of 10 key points are shown by Fig. \ref{fig:Defect}.  Atoms with ordered configuration  of $\gamma$ phase grains have been removed in Fig. \ref{fig:Defect}(a) and (b),\ $\alpha_2$ phase and defects inside grains have been left. Similarly, $\gamma$ phase grain have been removed in Fig. \ref{fig:Defect}(c) and (d), the defects of $\alpha_2$ phase have been left.  The results show that, at stage \uppercase\expandafter{\romannumeral1}, the structure of material is under typical elastic deformation, the size of simulation box enlarged due to the loading. In this stage, deformation of the two phase are compatible. Emission of dislocation and evolution of defects initiated at the end of stage \uppercase\expandafter{\romannumeral1}. A  great number of dislocation emitted inside $\gamma$ phase at earlier part of stage \uppercase\expandafter{\romannumeral2}, however, the dislocation inside $\alpha_2$ phase was emitted at the end of stage \uppercase\expandafter{\romannumeral2} is shown by Fig. \ref{fig:Defect}(c).  
\begin{figure}[ht] 
	\centering
...
\DIFdelbegin \DIFdel{Main reason for the unequal strain partitioning between the $\alpha_2$ and $\gamma$ phase is  the strong plastic anisotropy of  $\alpha_2$ phase. }\DIFdelend TEM examinations performed on tensile tested lamellar alloys have revealed that the limited plasticity of  $\alpha_2$ phase is carried by local slip of dislocations with the Burgers vector $1/3[11\overline{2}0]$ prism planes shown by Fig. \ref{fig:Defect}(b), which is by far the easiest slip system in $\alpha_2$ single crystals.  Dislocation neighboring interfaces often needs to be transformed. For example, an ordinary 1/2 [110] dislocation gliding in one $\gamma$ grain has to be converted into $[101]$ super dislocation when the double Burgers vectors gliding in an adjacent $\gamma$ grain. At the $\alpha_2/\gamma$ interface, dislocations existing in  $D0_{19}$ structure  transform into dislocations, which is consistent with the $\rm{L1_0}$ structure. These core transformations are associated with the change of the dislocation line energy because of the differences of the length and the shear module.  Pyramidal slip of the $\alpha_2$ phase is required  when slip is forced to cross $\alpha_2$ phase, this process needs an extremely high shear stress. \DIFaddbegin \DIFadd{Two components($\alpha_2$ and $\gamma$ phase) exhibit different properties in two phase alloy comparing they are single phase alloy respectively. $\gamma$ phase is the major source of dislocation in two phase alloy, that is in good agreement with experiments\mbox{%DIFAUXCMD
\cite{Singh2006a,Appel1998a,Singh2006}}%DIFAUXCMD
. When global strain is greater than 0.07, the reason why Ti3Al exhibits less plastic deformation is the absence of twinning in Ti3Al under tensile loading, which have been observed by experiments\mbox{%DIFAUXCMD
\cite{sastry1980plastic}}%DIFAUXCMD
. The deformation $\rm{Ti_3Al}$ is mainly constrained by loading conditions, as compression loading is applied, Ti3Al activates more moveable dislocation, thus exhibits better elasticity\mbox{%DIFAUXCMD
\cite{intro-structure}}%DIFAUXCMD
.
}\DIFaddend 
...
(1) \DIFdelbegin \DIFdel{Major deformation component of the two phase TiAl alloy is }\DIFdelend \DIFaddbegin \DIFadd{Deformation behavior of two types of grain inside $\alpha_2+\gamma$ two phase Ti-Al alloy are different due to their crystal structure. $\alpha_2$ phase $\rm{Ti_3Al}$ grain is easy to be deformed during elastic stage.}\\
\DIFadd{(2) During yield stage, majority of dislocations in two phase TiAl alloy activated inside }\DIFaddend $\gamma$ phase, $\alpha_2$ phase is harder to \DIFdelbegin \DIFdel{be deformed}\DIFdelend \DIFaddbegin \DIFadd{slip due to its hcp structure}\DIFaddend , this inhomogeneity results in cracks at interface of the two phase. \\
(\DIFdelbegin \DIFdel{2}\DIFdelend \DIFaddbegin \DIFadd{3}\DIFaddend ) Effect of  void on the strength of  \DIFdelbegin \DIFdel{the }\DIFdelend two phase alloy is sensitive to the location of void. Void inside grain has detraction to the strength of material because the strengthening mechanism similar to second phase particle. Void at $\alpha_2 / \gamma$ boundary is most risky situation for two phase alloy because of fast fracture along boundary.

\reftitle{References}
\DIFaddbegin 

\DIFaddend \bibliography{ref.bib}
\DIFdelbegin %DIFDELCMD < \funding{This research was funded by the National Natural Science Foundation of China (No.51665030, No.51865027) and the Program for Changjiang Scholars and Innovative Research Team in Universities of the Ministry of Education of China (No. IRT-15R30), the Hongliu First-class  Disciplines Development Program of Lanzhou University of Technology.}
%DIFDELCMD < %%%
\DIFdelend 


\DIFaddbegin \funding{This research was funded by the National Natural Science Foundation of China (No.51665030, No.51865027) and the Program for Changjiang Scholars and Innovative Research Team in Universities of the Ministry of Education of China (No. IRT-15R30), the Hongliu First-class  Disciplines Development Program of Lanzhou University of Technology.}
